...
Sete configurações comparadas. Todas usam PPO ou variantes; as diferenças estão em como atacam um ou mais dos três fatores acima.
...
Cada configuração roda com 3 seeds (0, 1, 2) e é avaliada em todos os três tamanhos. A config `mapcnn_bc_pbrs` rodou com 1 seed (interrompida após o seed 0 confirmar regressão no 20x20 — discussão na seção de Análise).
...
| Algoritmo (`maskable_v3`) | MaskablePPO + `MultiInputPolicy` (com action masking) |
| `ent_coef` | 0.05 (igual upstream); `maskable_v3` usa schedule linear 0.02 → 0.001 |
| `device` | cpu (4 primeiras configs), cuda (`curriculum_recurrent_v2`, `mapcnn_bc_pbrs`, `maskable_v3`) |
...
| `n_steps` | 128 default (4 primeiras configs), 512 (`curriculum_recurrent_v2`), 1024 (`mapcnn_bc_pbrs`, `maskable_v3`) |
...
| `gamma` (`mapcnn_bc_pbrs`, `maskable_v3`) | 0.999 (long-horizon: 1000 passos no 20x20) |
...
| Observação (`maskable_v3`) | enriched 5x5 (mesmo que `curriculum_enriched`) + `action_masks()` que oculta ações que batem em parede/obstáculo |
...
| Reward (`maskable_v3`, treino) | terminal full coverage **+60** (era +10), truncation **0** (era −5), step penalty 0 quando coverage ≥ 0.80; eval usa o reward upstream (sem redesign) |
| Net arch (`maskable_v3`) | `[256, 256]` (default 64x64 nas outras MLP) |
| Seeds | 0, 1, 2 |
...
Comparando as cinco configurações RL nas células-chave (linha de comparação direta com o enunciado):

| Treinado em ↓ \ Avaliado em → | Baseline | Curriculum | Enriched | Recurrent (CPU) | Recurrent v2 (GPU) |
|---|---|---|---|---|---|
| 5x5 → 10x10 | 14.0% | 14.0% | **69.7%** | 0.0% | 0.0% |
| 10x10 → 10x10 | 64.3% | 71.3% | **77.3%** | 1.3% | 10.0% |
| 20x20 → 10x10 | 47.7% | 64.7% | **73.0%** | 19.3% | 30.7% |
| 20x20 → 20x20 | 0.3% | 0.3% | **9.0%** | 0.0% | 0.0% |
| 5x5 → 5x5 | 92.7% | 92.7% | 91.3% | 83.0% | 83.7% |
