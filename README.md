...
A APS pede uma estratégia que faça um agente PPO treinado no problema de Coverage Path Planning (CPP) generalizar entre tamanhos de grid (5x5, 10x10 e, como bônus, 20x20) preservando a observabilidade parcial. O baseline do enunciado treina em 5x5 e degrada quando avaliado em grids maiores. Aqui investigamos seis configurações de RL (mais dois baselines clássicos não-learning para contexto) para atacar essa degradação.
...
Seis configurações comparadas. Todas usam PPO; as diferenças estão em como atacam um ou mais dos três fatores acima.
...
| `device` | cpu (4 primeiras configs), cuda (`curriculum_recurrent_v2`, `mapcnn_bc_pbrs`) |
...
| `n_steps` | 128 default (4 primeiras configs), 512 (`curriculum_recurrent_v2`), 1024 (`mapcnn_bc_pbrs`) |
...
| `gamma` (`mapcnn_bc_pbrs`) | 0.999 (long-horizon: 1000 passos no 20x20) |
