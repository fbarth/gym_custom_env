...
#
# Improvement over v1: the observation now includes the position of the
# NEAREST unvisited free cell (Manhattan distance).  This gives the agent
# an explicit "sub-goal" to pursue when most of the grid is already covered,
# directly addressing the "last cell" problem where a deterministic policy
# loops without finding the remaining uncovered cells.
#
# Reward function (same as v1, inspired by actor-critic CPP literature):
...
#    nearest_unvisited_x, nearest_unvisited_y, (2 values – agent pos when done)
...
class GridWorldCPPEnvV2(gym.Env):
    ...
    def __init__(self, render_mode=None, size: int = 5, obs_quantity: int = 3, max_steps: int = 400):
        self.size = size
        ...
        self._nearest_unvisited = np.array([0, 0], dtype=int)
        self._free_cells_count = 0

        # Observation: agent pos (2) + neighbors (4) + nearest unvisited (2) + visited grid (size*size)
        obs_size = 2 + 4 + 2 + size * size
    ...
    def _get_nearest_unvisited(self):
        """Return the position of the nearest unvisited free cell (Manhattan distance).
        Returns agent's own position when all cells are covered (terminal state)."""
        best = None
        best_dist = float('inf')
        for x in range(self.size):
            for y in range(self.size):
                if self._visited_grid[x, y] == 0:
                    cell = np.array([x, y])
                    if not any(np.array_equal(cell, obs) for obs in self.obstacles_locations):
                        dist = abs(x - self._agent_location[0]) + abs(y - self._agent_location[1])
                        if dist < best_dist:
                            best_dist = dist
                            best = cell
        return best if best is not None else self._agent_location.copy()

    def _get_obs(self):
        flattened = []
        flattened.extend(self._agent_location)
        flattened.extend(self._neighbors)
        flattened.extend(self._nearest_unvisited)
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        self._set_neighbors()
        self._nearest_unvisited = self._get_nearest_unvisited()
    ...
    def _render_frame(self):
        if self.window is None and self.render_mode == "human":
            ...
            pygame.display.set_caption("GridWorld CPP v2 – Nearest Unvisited")
        ...
        # Nearest unvisited cell – yellow highlight
        nu = self._nearest_unvisited
        if not np.array_equal(nu, self._agent_location):
            pygame.draw.rect(
                canvas,
                (255, 220, 50),
                pygame.Rect(pix * nu[0], pix * nu[1], pix, pix),
            )
