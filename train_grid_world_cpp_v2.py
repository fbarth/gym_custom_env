#
# python train_grid_world_cpp_v2.py <train|test|run>
#
...
if len(sys.argv) < 2 or sys.argv[1] not in ['train', 'test', 'run']:
    print("Usage: python train_grid_world_cpp_v2.py <train|test|run>")
    sys.exit(1)
...
try:
    gym.register(
        id="gymnasium_env/GridWorldCPPV2-v1",
...
if mode == 'train':
    print("--- Starting Training (CPP v2 – nearest unvisited cell) ---")
    env = gym.make(
        "gymnasium_env/GridWorldCPPV2-v1",
        size=DIM,
    ...
    log_dir = f'log/ppo_cpp_v2_{DIM}_{OBSTACLES}_{MAX_STEPS}_{ENTROPY_COEF}_{timestamp}'
    model_path = f'data/ppo_cpp_v2_{DIM}_{OBSTACLES}_{MAX_STEPS}_{ENTROPY_COEF}_{timestamp}.zip'
